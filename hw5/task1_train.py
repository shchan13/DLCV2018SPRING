...
def classsifier(in_dim, n_classes):
    ...
    plot_model(temp_model, to_file='classsifier.png', show_shapes=True)
...
def main():
    ...
    def data_generator(batch_size, latent_dim, n_classes, data_num, npy_dir):
        step_idx = 0
        __latent = np.zeros((batch_size, latent_dim))
        __label = np.zeros((batch_size, n_classes))
        ...
        while(True):
            ...
            for num in range(batch_size):
                __latent[num,:] = np.load(npy_dir+'feature_'+str(idx[step_idx])+'.npy')
                __label[num,:] = np.load(npy_dir+'label_'+str(idx[step_idx])+'.npy')
                step_idx += 1
    ...
    npy_dir = 'task1/npy/'
    val_npy_dir = 'task1/val_npy/'
    
    history_path = './task1/history_1.pickle'
    model_path = './task1/model/inceptionV3_classifier.hdf5'
    ...
    history = class_model.fit_generator(data_generator(batch_size, latent_dim, n_classes, train_num, npy_dir),
                                        steps_per_epoch = train_num // batch_size,
                                        epochs = epochs,
                                        validation_data = data_generator(batch_size, latent_dim, n_classes, val_num, val_npy_dir),
    ...
    val_data = np.zeros((val_num, latent_dim))
    val_label = np.zeros((val_num, n_classes))
    for i in range(val_num):
        val_data[i,:] = np.load(val_npy_dir+'feature_'+str(i+1)+'.npy')
        val_label[i,:] = np.load(val_npy_dir+'label_'+str(i+1)+'.npy')

    val_result = class_model.evaluate(val_data, val_label, verbose=1, batch_size=8)
    print('val loss = ', val_result[0])
    print('val acc = ', val_result[1])
